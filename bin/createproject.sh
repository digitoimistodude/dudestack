...
if [ ! -f /usr/local/etc/my.cnf.d ]; then
  echo "${yellow}Ensuring mysql is on (macOS)${txtreset}"
...
echo "${yellow}Generating HTTPS cert for project, attempt to run mkcert on vagrant...${txtreset}"
WP_CLI_PREFIXmkdir -p /var/www/certs && WP_CLI_PREFIXcd /var/www/certs && WP_CLI_PREFIXmkcert "$PROJECTNAME.test"
