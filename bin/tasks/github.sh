...
if grep -q "GITHUB_COMPANY_USERNAME" ${ENV_FILE}; then
  ...
  curl -u "'`echo $GITHUB_COMPANY_USERNAME_ENV`'":"'`echo $GITHUB_ACCESS_TOKEN_ENV`'" https://api.github.com/orgs/YOUR_GITHUB_COMPANY_USERNAME/repos -d '{"name": "'${PROJECTNAME}'","auto_init": false,"private": true,"description": "A repository for '${PROJECTNAME}' site"}'
